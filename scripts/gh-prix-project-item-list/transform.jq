...
def extract_project_item:
  . | {
        ...
        deadline: (.deadline | if . then .value else null end),
        impact: (.impact | if . then .value else null end),
        scope: (.scope | if . then .value else null end),
        severity: (.severity | if . then .value else null end),
        risk: (.risk | if . then .value else null end),
        footprint: (.footprint | if . then .value else null end),
        complexity: (.complexity | if . then .value else null end),
